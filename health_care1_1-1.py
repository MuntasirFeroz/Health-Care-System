...
'''
 ...
 Here we will predict whether a bank note is authentic or fake depending upon
 the four different attributes of the image of the note. The attributes are
 Variance of wavelet transformed image, curtosis of the image, entropy,
 and skewness of the image.
...
Image(graph.create_png('D:\Myworkplace\Python\health_care\decisiontree'))

'''
from sklearn.tree import export_graphviz
import graphviz # to install this package in conda "conda install python-graphviz"
                #use th command in ""
dot_data = tree.export_graphviz(classifier, out_file=None)
graph = graphviz.Source(dot_data)
#graph.render("dataset")
dot_data = tree.export_graphviz(classifier, out_file=None,
                      feature_names=['TEMPERATURE','PULSE'],
